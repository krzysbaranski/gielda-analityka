...
pivot_value_total.drop(columns=['Total']).plot.area(stacked=True)
plt.title('Segmentacja wartości portfela wg pozycji (skumulowana)')
...
plt.figure(figsize=(14, 7))
pivot_value_per_unit.plot()
plt.title('Wartość jednostkowa (wartość/ilość) dla każdej pozycji')
plt.ylabel('Wartość jednostkowa (PLN)')
plt.xlabel('Data')
plt.legend(loc='upper left')
plt.tight_layout()
plt.show()
